...
public class AdministrativeApprovalAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private List<AdministrativeApproval.ResultsBean> datas;
...
//    private static final String HOST = "http://www.jcodecraeer.com";
    ...
    public AdministrativeApprovalAdapter(List<AdministrativeApproval.ResultsBean> datas, Context context) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<AdministrativeApproval.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<AdministrativeApproval.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final AdministrativeApproval.ResultsBean item = datas.get(position);

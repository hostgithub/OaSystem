...
public class SendFileHasDealAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private List<DispatchHasDeal.ResultsBean> datas;
...
//    private static final String HOST = "http://www.jcodecraeer.com";
    ...
    public SendFileHasDealAdapter(List<DispatchHasDeal.ResultsBean> datas, Context context) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<DispatchHasDeal.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<DispatchHasDeal.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final DispatchHasDeal.ResultsBean item = datas.get(position);

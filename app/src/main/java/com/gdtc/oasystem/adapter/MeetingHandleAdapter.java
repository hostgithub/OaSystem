...
public class MeetingHandleAdapter extends RecyclerView.Adapter<BaseViewHolder> {
    private static final int TYPE_FOOTER = 0;
    private static final int ITEM_NOIMAGE = 1;
    private static final int ITEM_HASIMAGE = 2;

    private List<MeetingHandle.ResultsBean> datas;
...
//    private static final String HOST = "http://www.jcodecraeer.com";
    ...
    public MeetingHandleAdapter(List<MeetingHandle.ResultsBean> datas, Context context) {
        this.datas = datas;
        this.mContext = context;
    }

    public void replaceAll(List<MeetingHandle.ResultsBean> elements) {
        if (datas.size() > 0) {
            datas.clear();
        }
    ...
    public void addAll(List<MeetingHandle.ResultsBean> elements) {
        datas.addAll(elements);
        notifyDataSetChanged();
    }
    @Override
    ...
    public void onBindViewHolder(BaseViewHolder holder, final int position) {
        if (!(viewFooter != 0 && position == getItemCount() - 1)) {
            ...
            final MeetingHandle.ResultsBean item = datas.get(position);
